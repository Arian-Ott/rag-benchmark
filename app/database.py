...
class DocumentDB:
    def __init__(self):
        ...
        self.router.add_api_route("/files/upload_pdf", self.upload_file,
                                  methods=[
                                      "POST"], tags=["Files"])
        self.router.add_api_route("/files/list_files", self.list_files,
                                  methods=[
                                      "GET"], tags=["Files"])
        self.router.add_api_route("/files/get_file/{file_id}", self.get_file,
                                  methods=["GET"], tags=["Files"])
        self.router.add_api_route(
            "/files/delete_file/{file_id}", self.delete_file, methods=[
                "DELETE"], tags=["Files"]
        )
        self.router.add_api_route("/db/add_user", self.create_user,
                                  methods=["POST"], tags=["CouchDB"])
    ...
    async def create_user(self, data: UserCreation = Body(...)):
        ...
        if not (hashlib.sha3_512(data.authorisation.encode("utf-8")).hexdigest()
